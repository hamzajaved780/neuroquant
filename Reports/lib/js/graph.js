...
var SELECT_CLUSTER_DIV = GRAPH_MESSAGE_PREFIX + "Select a cluster to display." + GRAPH_MESSAGE_SUFFIX;
var SELECT_BLOCK_DIV = GRAPH_MESSAGE_PREFIX + "Select a block to display." + GRAPH_MESSAGE_SUFFIX;

var GRAPH_CONST = {
    SPV:  { name: "spv_graph",  elemId: "spg",   gid: "SPG"   },
    SV:   { name: "sv_graph",   elemId: "svg",   gid: "SVG"   },
    CSPV: { name: "cspv_graph", elemId: "cspg",  gid: "CSPG"  },  // TODO: renamed to FV
    LMEM: { name: "lmem_graph", elemId: "lmemg", gid: "LMEMG" },
    BV:   { name: "bv_graph",   elemId: "bvg",   gid: "BVG"   },
    PV:   { name: "pv_graph",   elemId: "pvg",   gid: "PVG"   }
};
// Graph variables
var spv_graph;
var pv_graph;
 ...
 *  graphType  : allowed values are [SPV|CSPV|LMEM|EMEM|BV|PV]. Values same as GRAPH_CONST
 ...
 *  ememFlag   : [showall|true|false] SPV, CSPV - true means show external memory and LSU's if exist
    ...
    function addCheckBox() {
        ...
        if ((graphType == "SPV") || (graphType == "CSPV")) {
            nodeTypes.forEach(function (nt) {
                switch (nt) {
                    case "inst": menu += CHECK_BOX_PREFIX + nt + "' onClick='" + graph + ".resetVisibleLinks()'>&nbspControl&nbsp&nbsp";
                        break;
...
function renderNoGraphForType(graph,title,type,details,message){
    ...
    switch(type){
        ...
        case "no_nodes":
            ...
            break;
        case "choose_cluster":
            contentText = SELECT_CLUSTER_DIV;
            break;
        case "choose_block":
            contentText = SELECT_BLOCK_DIV;
            break;
        case "message":
...
function renderGraph(graphJSON, graphType, graphID) {
    if (top_node_id === graphID) return;  // do nothing if user clicks the same ID
    top_node_id = graphID;

    if ((graphType === "SPV" && spv_graph) || (graphType == "SV" && sv_graph) || (graphType === "PV" && pv_graph) || (graphType === "BV" && bv_graph)) {
        // Clear previous selected details
        switch (graphType) {
            case "SPV":
              spv_graph.refreshGraph();
              break;
            case "SV":
              sv_graph.refreshGraph();
              break;
            case "PV":
              pv_graph.refreshGraph();
              break;
            case "BV":
              bv_graph.refreshGraph();
              break;
        }
    }
    ...
    if (graphJSON === undefined) {
        if (graphType == "PV") {
            renderNoGraphForType("#" + GID, "", "choose_cluster", "");
            return;
        } else if (graphType == "BV") {
            renderNoGraphForType("#" + GID, "", "choose_block", "");
            return;
        }
    } else if (graphJSON.nodes === undefined) {
    ...
    setTimeout(function() {
        switch (graphType) {
            case "SPV":
                spv_graph = new StartGraph(graphJSON, graphType);
                spv_graph.zoomToFit(0);
                break;
            case "SV":
                sv_graph = new StartGraph(graphJSON, graphType);
                sv_graph.zoomToFit(0);
                break;
            case "PV":
                pv_graph = new StartGraph(graphJSON, graphType);
                pv_graph.zoomToFit(0);
                break;
            case "BV":
                bv_graph = new StartGraph(graphJSON, graphType);
                bv_graph.zoomToFit(0);
                break;
        }
