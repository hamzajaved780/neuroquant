...
var MENU = {
    ...
    VIEWERS:       { value: 4, name: "System Viewers", submenu: ["SPV", "SV", "CSPV", "divider", "LMEM", "BV", "PV"] },
...
var VIEWS = {
    ...
    SPV:         { value: 40, name: "System Viewer",               flow: FLOWS.OPENCL, id: "spv"   },  // TODO: both
    SV:          { value: 40, name: "System Viewer",               flow: FLOWS.HLS,    id: "system" }, // TODO: replace SPV
    CSPV:        { value: 41, name: PREFIX_PAD + " Viewer",        flow: FLOWS.HLS,    id: "comp"  },  // TODO: both
    ...
    BV:          { value: 44, name: "Block Viewer (beta)",                flow: FLOWS.BOTH, id: "bv"},
    PV:          { value: 45, name: "Cluster Viewer (beta)",             flow: FLOWS.BOTH,    id: "pipeline" },
...
var detailIndex = 0;
var curFile;
var lmem_graph;  // TODO: move to graph.js after refactored
var cspv_graph;  // TODO: move to graph.js after refactored
var detailOptValues = [];
...
function goToView(viewId, update) {
    ...
    switch (view) {
      case VIEWS.SPV:
        if (isValidSystemViewer) renderGraph(mavJSON, viewId, 0);
        else $('#SPG').html("&nbsp" + view.name + " data is invalid!");
        break;
      case VIEWS.SV:
        if (isValidSoTViewer) renderGraph(systemJSON, viewId);
        else $('#SVG').html("&nbsp" + view.name + " data is invalid!");
        break;
      case VIEWS.LMEM:
      ...
      case VIEWS.CSPV:
        if (!cspv_graph && isValidSystemViewer) {
            var hasComp = addHierarchyTree(VIEWS.CSPV);
            if (hasComp) {
        ...
        break;
      case VIEWS.PV:
        if (isValidPipelineViewer) addHierarchyTree(VIEWS.PV);
        else $('#' + GRAPH_CONST.PV.gid).html("&nbsp" + view.name + " data is invalid!");
        break;
      case VIEWS.BV:
        if (isValidPipelineViewer) addHierarchyTree(VIEWS.BV);
        else $('#' + GRAPH_CONST.BV.gid).html("&nbsp" + view.name + " data is invalid!");
        break;
      case VIEWS.DOT:
...
function addReportColumn(reportEnum) {
    ...
    switch (reportEnum) {
      ...
      case VIEWS.INCREMENTAL:
        ...
        break;
      case VIEWS.SPV:
        report = addViewerPanel(reportEnum, "layers", "SPG");
        break;
      case VIEWS.SV:
        report = addViewerPanel(reportEnum, "layers-system", "SVG");
        break;
      case VIEWS.CSPV:
        report = addTreePanel(reportEnum);
        report += "<div class=\"col col-sm-9\" id=\"report-pane-col2\">";
        report += addViewerPanel(reportEnum, "layers-comp", "CSPG");
        break;
      case VIEWS.LMEM:
        report = addTreePanel(reportEnum);
        report += "<div class=\"col col-sm-9\" id=\"report-pane-col2\">";
      ...
      case VIEWS.PV:
        report = addTreePanel(reportEnum);
        report += "<div class=\"col col-sm-9\" id=\"report-pane-col2\">";
        report += addViewerPanel(reportEnum, "layers-pipeline", "PVG");
        break;
      case VIEWS.BV:
        report = addTreePanel(reportEnum);
        report += "<div class=\"col col-sm-9\" id=\"report-pane-col2\">";
        report += addViewerPanel(reportEnum, "layers-bv", "BVG");
        break;
      case VIEWS.DOT:
        report += "<table style='width:100%'><tr><td class='panel-heading-text'>";
        report += reportEnum.name;
...
setReportPaneHeight()
{
    ...
    if (view == VIEWS.SV) {
        $('#SVG').css('height', panelHeight - panelHeadingHeight);
        $('#svg').css('height', panelHeight - panelHeadingHeight);
        $('#svg').css('width', $('#report-pane').innerWidth());
    ...
    if (view == VIEWS.PV) {
        $('#PVG').css('height', panelHeight - panelHeadingHeight);
        $('#pvg').css('height', panelHeight - panelHeadingHeight);
        $('#pvg').css('width', $('#report-pane').innerWidth());
        $('ul.fancytree-container').css('height', panelHeight - panelHeadingHeight);
    }

    if (view == VIEWS.BV) {
        $('#BVG').css('height', panelHeight - panelHeadingHeight);
        $('#bvg').css('height', panelHeight - panelHeadingHeight);
        $('#bvg').css('width', $('#report-pane').innerWidth());
        $('ul.fancytree-container').css('height', panelHeight - panelHeadingHeight);
    }
...
{
    if (view == VIEWS.SPV || view == VIEWS.SV || view == VIEWS.LMEM || view == VIEWS.CSPV || view == VIEWS.PV || view == VIEWS.BV) {
        if (!details) return;
...
syncEditorPaneToLine( line, filename )
{
    ...
    if (index === undefined) {
      ...
      index = fileIndexMap[filename.replace(/^\.\//, '')];
    }
