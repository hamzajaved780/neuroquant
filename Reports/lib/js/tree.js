...
function addHierarchyTree(view) {
  ...
  function getMemEntry(node, kernelName) {

    addMemType(node.type);
    var entryIcon = (node.type === "memsys") ? MEMORY_ICON : ROM_ICON;
    var memEntry = { title: node.name, type: node.type, kernel: kernelName, expanded: false, icon: ICON_LOC + entryIcon, children: [] };
    var bankID = 0;
    while (bankID < node.children.length) {
        ...
        var bankEntry = { title: bankName, type: bankNode.type, selected:isCheck, bank: bankNode.name, lmem: node.name, kernel: kernelName, expanded: true, icon: false };
  ...
  if (treeNodes.nodes.length !== 0) {
    ...
    treeNodes.nodes.forEach(function (element) {
      ...
      if (element.type == "kernel" || element.type == "component") {
        ...
        var kernelEntry = { title: kernelName, type: element.type, hideCheckbox: true, expanded: true, icon: ICON_LOC + KERNEL_ICON, children: [] };
        if (VIEWS.CSPV !== undefined && viewValue === VIEWS.CSPV.value) {
          hierList.push(kernelEntry);
          return;
        }
        ...
        element.children.forEach(function (node) {
          if (viewValue == VIEWS.PV.value) {  // TODO: change to BV when BV is added
            if (node.type == "bb") {
              ...
              // Block view TO BE ADDED
              // if (viewValue === VIEWS.BV.value) {
              //   kernelEntry.children.push(bbEntry);
              //   hierList.push(kernelEntry);
              //   return;
              // }
          ...
          else if (viewValue == VIEWS.BV.value) {  // TODO: change to BV when BV is added
            if (node.type == "bb") {
              // Add all blocks to function, block and cluster viewer
              var bb_name = node.name;
              var bb_entry = { title: bb_name, type: node.type, nodeId: node.id, hideCheckbox: true, expanded: true, icon: ICON_LOC + KERNEL_ICON, children: [] };
              // Block view TO BE ADDED
              // if (viewValue === VIEWS.BV.value) {
              //   kernelEntry.children.push(bbEntry);
              //   hierList.push(kernelEntry);
              //   return;
              // }
              kernelEntry.children.push(bb_entry);
            }
          }
            ...
            if (node.type == "memtype" && node.name == "Local Memory") {  
              ...
              node.children.filter(function(n){ return (n.type === "memsys" || n.type === "romsys");}).forEach(function (lmemNode) {
                kernelEntry.children.push(getMemEntry(lmemNode, kernelName));
              });
              // Add all the local memory groups
              node.children.filter(function(n){ return n.type === "memgroup";}).forEach(function (lmemGroup) {
                var memGroupEntry = { title: lmemGroup.name, type: lmemGroup.type, kernel: kernelName, hideCheckbox: true, expanded: true, icon: ICON_LOC + MEMORY_ICON, children: [] };
                lmemGroup.children.forEach(function(lmemNode){
                  memGroupEntry.children.push(getMemEntry(lmemNode, kernelName));
                });
                kernelEntry.children.push(memGroupEntry);
              });
              ...
              node.children.filter(function(n){ return ((n.type === "reg") || (n.type === "unsynth") || (n.type === "untrack"));}).forEach(function(mnode) {
                ...
                var nodeEntry = { title: mnode.name, type: mnode.type, kernel: kernelName, hideCheckbox: true, expanded: true, icon: iconLocation, details: [], children: [] };
                nodeEntry.details = mnode.details;
                kernelEntry.children.push(nodeEntry);
              });

            } // end backward compitable
            else if (node.type == "memsys") {
              // Add the local memory system, i.e. banks and replication
              kernelEntry.children.push(getMemEntry(node, kernelName));
            } 
            else if (node.type == "memgroup") {
              // Add inlined and unrolled copies
              var memGroupEntry = { title: node.name, type: node.type, kernel: kernelName, hideCheckbox: true, expanded: true, icon: ICON_LOC + MEMORY_ICON, children: [] };
              ...
              var nodeEntry = { title: node.name, type: node.type, kernel: kernelName, hideCheckbox: true, expanded: true, icon: iconLocation, details: [], children: [] };
    ...
    $(elemId).fancytree({
      ...
      click: function (event, data) {
        var curElemID = this.getAttribute('id');
        // TODO: remove the check CSPV when switch to BOTH flows
        if (VIEWS.CSPV !== undefined && curElemID === getTreeIDName(VIEWS.CSPV.id)) {
        ...
        else if (curElemID === getTreeIDName(VIEWS.PV.id)) {
          var nodeId = data.node.data.nodeId;
          $("#PVG").parent().find(".currentEntity").text(": " + data.node.parent.title + " > " + data.node.title);
          renderGraph(pipelineJSON[nodeId], "PV", nodeId);
        }
        else if (curElemID === getTreeIDName(VIEWS.BV.id)) {
          var node_id = data.node.data.nodeId;
          $("#BVG").parent().find(".currentEntity").text(": " + data.node.title);
          renderGraph(blockJSON[node_id], "BV", node_id);
        }
